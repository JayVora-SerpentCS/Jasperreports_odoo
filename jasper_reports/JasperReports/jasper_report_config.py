...
class Report:
    ...
    def execute(self):
        ...
        rep_xml_set = self.env["ir.actions.report"].search(
            [("report_name", "=", self.name[7:]), ("report_file", "ilike", ".jrxml")]
        )
        ...
        for data in rep_xml_set:
            ...
            for sub_report_info in self.report.subreports:
                ...
                if sub_report.language == "xpath":
                    ...
                    if sub_report_info["pathPrefix"]:
                        message += "with prefix %s " % sub_report_info["pathPrefix"]
                    else:
            ...
            pages = self.execute_report(data_file, output_file, sub_report_data_files)
    ...
    def user_name(self):
        ...
        user_name = (
            self.env["ir.config_parameter"].sudo().get_param("db_user")
            or self.system_user_name()
        )
        return tools.config["db_user"] or user_name

    def password(self):
        # To allow all users to get db_password we have to call it with sudo
        password = self.env["ir.config_parameter"].sudo().get_param("db_password") or ""
