...
class BrowseDataGenerator(AbstractDataGenerator):
    ...
    def languages(self):
        ...
        ids = self.pool.get('res.lang').search(self.cr, self.uid,
                                               [('translatable', '=', '1')])
        self._languages = self.pool.get('res.lang').read(self.cr, self.uid,
                                                         ids, ['code'])
    ...
    def valueInAllLanguages(self, model, id, field):
        ...
        for language in self.languages():
            ...
            if(model._columns[field]._type == 'selection' and
               model._columns[field].selection):
                field_data = model.fields_get(self.cr, self.uid,
                                              allfields=[field],
                                              context=context)
                values[language] = dict(field_data[field
                                                   ]['selection'
                                                     ]).get(values[language],
                                                            values[language])
    ...
    def generateIds(self, record, relations, path, currentRecords):
        ...
        for relation in unrepeated:
            ...
            if root == 'Attachments':
                ...
                value = self.pool.get('ir.attachment').browse(self.cr,
                                                              self.uid, ids,
                                                              self.context)
            elif root == 'User':
...
# for each model id we've been asked to show. If there are any elements
# in the OPENERP_RELATIONS
...
class XmlBrowseDataGenerator(BrowseDataGenerator):
    def generate(self, fileName):
        ...
        for record in self.pool.get(self.model).browse(self.cr,
                                                       self.uid, self.ids,
                                                       self.context):
        ...
        for field in unrepeated:
            ...
            if isinstance(value, orm.browse_record_list):
                ...
                if currentPath in records:
                    self.generateXmlRecord(records[currentPath],
                                           records, fieldNode,
                                           currentPath, fields2)
                else:
            ...
            if field == 'id':
                # Check for field 'id' because we can't find
                # it's type in _columns
                value = str(value)
            elif value == False:
                value = ''
...
class CsvBrowseDataGenerator(BrowseDataGenerator):
    ...
    # By default (if no OPENERP_RELATIONS property exists in the report)
    # a record will be created
    # for each model id we've been asked to show.
    # If there are any elements in the OPENERP_RELATIONS
    # list, they will imply a LEFT JOIN like behaviour on the rows to be shown.
    def generate(self, fileName):
        self.allRecords = []
        relations = self.report.relations()
        # The following loop generates one entry to allRecords list
        # for each record that will be created. If there are any
        # relations it acts like a LEFT JOIN against the main model/table.
        ...
        try:
            csv.QUOTE_ALL = True
            # JasperReports CSV reader requires an extra colon at
            # the end of the line.
    ...
    def generateCsvRecord(self, record, records, row, path, fields, sequence,
        ...
        for field in unrepeated:
            ...
            # In order not to change the way we detect many2one fields,
            # we simply check that the field is in self.report.fields()
            # and that's it.
            ...
            # The rest of field types must be converted into str
            if field == 'id':
                # Check for field 'id' because we can't find it's type
                value = str(value)
